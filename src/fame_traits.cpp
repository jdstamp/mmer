...
fame_traits_cpp(std::string plink_file, float additive_variance_component, float gxg_variance_component,
    ...
    MatrixXdr additive_effects = draw_normal_effects(n_additive,
                                                     additive_variance_component / n_additive);
    MatrixXdr epistatic_effects = draw_normal_effects(n_gxg,
    ...
    MatrixXdr error_component = draw_normal_effects(n_samples,
                                                    1 -
                                                    additive_variance_component - gxg_variance_component);
    ...
    for (int i = 0; i < n_gxg; ++i) {
            ...
            global_snp_index = -1;
            read_focal_snp(bed_file, epistatic_snp_genotype, gxg_snp_index, n_samples, n_snps,
                           global_snp_index);
            normalize_genotype(epistatic_snp_genotype, n_samples);
            epistatic_component = epistatic_component.array()
                    + focal_snp_genotype.array() * epistatic_snp_genotype.array() * epistatic_effects(i, 0);
    }
