...
context("C++ test yXXy with masking") {
  test_that("mailman algorithm reproduces naive implementation") {
    // given
    ...
    for (int i = 0; i < block_size; i++) {
      matrix_block.col(i) =
          matrix_block.col(i).array() - means(i, 0);
      matrix_block.col(i) =
          matrix_block.col(i).array() * stds(i, 0);
    }
...
context("C++ test major or minor allele count encoding gives same result") {
  test_that("Standard deviation and mean are equivalent for major or minor "
    ...
    for (int i = 0; i < block_size; i++) {
      stds_minor(i, 0) = 1 / sqrt((means_minor(i, 0) * (1 - (0.5 *
                                                             means_minor(i, 0)))));
      stds_major(i, 0) = 1 / sqrt((means_major(i, 0) * (1 - (0.5 *
                                                             means_major(i, 0)
                                                             ))));
    }
...
XXz_observed =
        compute_XXz(block_size, pheno, fame_means,
                    fame_stds, pheno_mask, genotype_mask_matrix, n_randvecs,
  ...
  test_that("masking gives the expected result for XXz") {
    ...
    XXz_observed =
        compute_XXz(block_size, pheno, fame_means,
                    ...
                    n_samples, 0, sum_op, genotype_block, yint_m, y_m,
                    block_size, yint_e, y_e, partialsums, false);
    // print XXz_expected and XXz_observed to cout
        std::cout << "XXz_expected: " << XXz_expected << std::endl;
        std::cout << "XXz_observed: " << XXz_observed << std::endl;
