...
void genotype::read_txt_naive(std::string filename, bool allow_missing) {
  ...
  if (!(iss >> Nsnp >> Nindv)) {
    cout << "ERROR: Header with number of SNPs and individuals not present"
         << endl;
    exit(-1);
  }
...
template <class T>
inline void printvector(vector<T> &t, string delim = " ",
                        bool newline = false) {
...
template <class T> inline void printvectornl(vector<T> &t, string delim = " ") {
...
void genotype::read_bed_mailman(std::istream &ifs, string filename,
                                bool allow_missing) {
...
void genotype::read_bed_mailman_nomissing(string filename) {
  ...
  for (int i = 0; i < Nsnp; i++) {
    ...
    for (int k = 0; k < ncol; k++) {
      ...
      // To handle missing data it is recommended to write a separate function.
      // This is easy to do. This will avoid the performance hit of checking for
...
void genotype::read_bed(std::istream &stream, string filename,
...
void genotype::read_plink(std::istream &ifs, std::string filenameprefix,
                          bool allow_missing, bool mailman_mode) {
...
double genotype::get_geno(int snpindex, int indvindex,
