...
To install `Rhdf5lib`, first
install the tool `BiocManager` from CRAN, then install the library using this tool.
...
To enable openMP, it may be necessary to configure the compiler flag `SHLIB_OPENMP_CXXFLAGS` in the `~/.R/Makevars` file.
...
The `src/Makevars` file configures the compiler flags and considers the 
`LDFLAGS` and `CPPFLAGS` from the `~/.R/Makevars` file.
