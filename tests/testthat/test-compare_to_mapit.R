test_that("Compare to MAPIT inference", {
  ...
  # test_phen <- read.table(pheno_file, header = T)
  # test_data <- read_plink(plink_file)
  # y <- test_phen$pheno
  # y <- y - mean(y)
  # X <- t(test_data$X)
  # X <- scale(X)
  ...
  # G <- t(t(G * xk) * xk)
  # I <- diag(1, n_samples, n_samples)
  # b <- cbind(rep(1, n_samples), xk)
  # P <- I - b %*% solve(t(b) %*% b) %*% t(b)
  # PKP <- P %*% K %*% P
  # PGP <- P %*% G %*% P
  #
  #
  ...
  # invS <- solve(S)
  # var_sig <- c()
  # V <- expected[1] * K + expected[2] * G + expected[3] * I
  # for (i in 1:3) {
  #   H <- invS[1, i] * K + invS[2, i] * G + invS[3, i] * I
  #   var_sig <- c(var_sig, 2 * t(y) %*% H %*% V %*% H %*% y)
  # }
  #
  # Py <- P %*% y
  # S11 <- sum(diag(PKP %*% PKP))
  # S21 <- sum(diag(PKP %*% PGP))
  # S31 <- sum(diag(PKP))
  # S22 <- sum(diag(PGP %*% PGP))
  # S32 <- sum(diag(PGP))
  # S33 <- sum(diag(P))
  # SP <- matrix(c(S11, S21, S31,
  #               S21, S22, S32,
  #               S31, S32, S33), nrow = 3)
  #
  # q1 <- t(Py) %*% PKP %*% Py
  ...
  # q3 <- t(Py) %*% Py
  # qP <- c(q1, q2, q3)
  # expectedP <- solve(SP, qP)
  # invS <- solve(SP)
  # var_sigP <- c()
  # V <- expectedP[1] * PKP + expectedP[2] * PGP + expectedP[3] * P
  # for (i in 1:3) {
  #   H <- invS[1, i] * PKP + invS[2, i] * PGP + invS[3, i] * P
  #   var_sigP <- c(var_sigP, 2 * t(Py) %*% H %*% V %*% H %*% Py)
  # }
  #
  # # when
